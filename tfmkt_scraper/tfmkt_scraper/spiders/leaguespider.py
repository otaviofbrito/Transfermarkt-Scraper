...
class LeagueSpider(CrawlSpider):
    ...
    rules = (
        Rule(LinkExtractor(restrict_xpaths='//tr[@class="odd" or @class="even"]/td/table/tr/td[2]/a', deny=[
             '/profil/spieler/', '/pokalwettbewerb/']), callback='parse_league_page'),
        Rule(LinkExtractor(
    ...
    custom_settings = {
        'ITEM_PIPELINES': {
            ...
            "tfmkt_scraper.pipelines.league.neo4j_league_pipeline.Neo4jLeaguePipeline": 420
        },
        'FEEDS': {
    ...
    def parse_league_page(self, response):
        ...
        mv = response.xpath(
            '//div[@class="data-header__box--small"]/a/text()[2]').get()
